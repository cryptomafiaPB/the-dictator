...
const Page = async () => {
  ...
  const totalPageviews = pageviews.reduce((acc, curr) => {
    return (
      acc +
      curr.events.reduce((acc: any, curr: any) => {
  ...
  const amtVisitorsToday = pageviews
    ...
    .reduce((acc, curr) => {
      return (
        acc +
        curr.events.reduce(
          (acc: any, curr: any) => acc + Object.values(curr)[0]!,
          0
        )
      );
    }, 0);
    ...
    for (let j = 0; j < day.events.length; j++) {
      ...
      if (country) {
        if (topCountriesMap.has(country)) {
          ...
          topCountriesMap.set(
            country,
            prevValue + (typeof value === "number" ? value : 0)
          );
        } else {
          topCountriesMap.set(country, typeof value === "number" ? value : 0);
        }
      }
    }
  }
